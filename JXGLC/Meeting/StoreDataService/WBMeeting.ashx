<%@ WebHandler Language="C#" Class="WBMeetingDataList" %>
...
public class WBMeetingDataList : YZServiceHandler
{
    //数据库连接
                ...
                if(Convert.ToString(stateItem["State"]["State"])=="approved" || Convert.ToString(stateItem["State"]["State"])=="running"){ 
                    ...
                    //根据每一条taskid查找对应的记录
                     using(SqlConnection sqlConn = new SqlConnection())
                     {
                         ...
                         sqlConn.Open();
                         string _sql = "select * from OA_ExternalMeeting_Meeting where [TaskID] = '" + task.TaskID + "' and";
                         ...
                         if(reader.Read())
                         {
                            ...
                            item["Title"] = reader.ReadString("Title");//会议名次
                            item["Time"] = reader.ReadDateTime("Time");//会议时间
                            item["Place"] = reader.ReadString("Place");//会议地点
                            item["HostUnit"] = reader.ReadString("HostUnit");//主办单位
