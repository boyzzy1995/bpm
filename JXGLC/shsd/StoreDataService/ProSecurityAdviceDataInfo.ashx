<%@ WebHandler Language="C#" Class="ProSecurityAdviceDataList" %>
...
public class ProSecurityAdviceDataList : YZServiceHandler
{
    //数据库连接
                ...
                if(Convert.ToString(stateItem["State"]["State"])=="approved" || Convert.ToString(stateItem["State"]["State"])=="running"){ 
                    ...
                    //根据每一条taskid查找对应的记录
                     using(SqlConnection sqlConn = new SqlConnection())
                     {
                         ...
                         sqlConn.Open();
                         string _sql = "select * from OA_SHSD_ProSecurityAdvice where [TaskID] = '" + task.TaskID + "' and";
