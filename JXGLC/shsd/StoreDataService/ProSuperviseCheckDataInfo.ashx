<%@ WebHandler Language="C#" Class="ProSuperviseCheckDataList" %>
...
public class ProSuperviseCheckDataList : YZServiceHandler
{
    //数据库连接
                ...
                if(Convert.ToString(stateItem["State"]["State"])=="approved" || Convert.ToString(stateItem["State"]["State"])=="running"){ 
                    ...
                    //根据每一条taskid查找对应的记录
                     using(SqlConnection sqlConn = new SqlConnection())
                     {
                         ...
                         sqlConn.Open();
                         string _sql = "select * from OA_SHSD_ProSuperviseCheck where [TaskID] = '" + task.TaskID + "' and";
                         ...
                         if(reader.Read())
                         {
                            ...
                            item["Number"] = reader.ReadString("Number");//编号
                            item["Title"] = reader.ReadString("Title");//项目名称
                            item["License"] = reader.ReadString("License");//许可文号
                            item["CheckProblem"] = reader.ReadString("CheckProblem");//检查发现问题
                            item["Time"] = reader.ReadDateTime("Time");//日期
